...
Consumidor NÃO DEVE especializar núcleo gerenciado por edição, cópia oficial simulada, patch oculto ou reescrita pós-sincronização. Especialização usa somente `agents.local.md`, `./.ia.rules/local/`, `./.ia.rules/hooks/` ou adaptador declarado: é subordinada, carregada quando aplicável e excluída de descoberta, lock, remoção e sobrescrita. Path declarado pelo manifesto validado pertence ao núcleo: colisão, autoria ambígua ou customização nesse path DEVE gerar ZIP de compressão máxima em `./agents-governance-backups/YYYY-MM-DD/`, raiz ignorada pelo Git, e ser substituída sem bloquear convergência; colisão fora do manifesto permanece intocada. O backup inclui instante, identidade/versão, é informado para exclusão humana e não integra lock/commit. `--force` é alias transitório sem autoridade adicional e não converte customização em regra. RCF que declare repositório construtor PODE autorizar edição da fonte distribuível, distinta da governança ativa. Terceiro importado só PODE ser processado após incorporação definitiva.

Formato vigente usa exclusivamente `./.ia.rules/`; a árvore predecessora está expurgada e não possui alias, leitura compatível ou presença no payload. Extensão nova reside em namespace próprio sob `./.ia.rules/local/`, `./.ia.rules/hooks/` ou adaptador declarado e DEVE constar do mapa local de preservação sem integrar o manifesto gerenciado do release.
...
Na ausência de regra mais específica: timeout local 30 s, rede 120 s, operação pesada 900 s; retry idempotente transitório 2 vezes com 1 s/3 s, demais 0; concorrência 1 para mutação e até 4 para leituras declaradas paralelizáveis; códigos 0/1/2/3/4/130 = sucesso/falha/entrada inválida/indisponível/não conformidade ou filtro/interrupção; entrada UTF-8, path relativo, ordenação estável, locale neutro e data ISO 8601 com `Z` ou deslocamento explícito. Override DEVE declarar valor, escopo e autoridade.

## MN-STATE — FT e retomada

Fluxo comportamental: solicitação → intenção → FTs normativa/código → commit de criação/conciliação → RCF → validação normativa → autorização humana explícita → código → validação técnica → memória → commits/push. FT possui `FT-NNN`, tipo, `criado_em` imutável e único em ISO 8601 com fuso, origem/vínculos, nome, escopo Técnico/Negócio, objetivo, prioridade, status, dependências, aceite, etapas, tarefas, decisões, verificações, comandos, hipóteses descartadas, falhas, pendências e próximo ponto; empate temporal usa o ID. Data legada sem fuso preserva instante/ID segundo o fuso histórico declarado e converge ao ser reaberta ou migrada. Norma e código não compartilham estado; criação não normatiza, conclusão normativa não autoriza código e precedência da FT mais nova substitui somente incompatibilidade material após conciliação. Memória é contínua; conclusão atualiza estado, valida, commita e publica quando autorizada/possível. Interrupção registra `[INTERROMPIDO_POR_LIMITACAO_DE_RECURSOS]` e só remove a flag após retomada validada.
