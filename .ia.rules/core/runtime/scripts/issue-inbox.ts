...
const DEFAULT_INBOX_DIR = path.join(ROOT_DIR, ".ia.rules", "local", "upstream", "inbox");
...
/** Executa receiveEvent no fluxo deste módulo; centraliza contrato reutilizável e preserva validações do chamador. */
...
/** Executa fetchIssue no fluxo deste módulo; centraliza contrato reutilizável e preserva validações do chamador. */
...
/** Executa processEvent no fluxo deste módulo; centraliza contrato reutilizável e preserva validações do chamador. */
...
/** Executa normalizeIssue no fluxo deste módulo; centraliza contrato reutilizável e preserva validações do chamador. */
...
/** Executa persistRecord no fluxo deste módulo; centraliza contrato reutilizável e preserva validações do chamador. */
...
/** Executa evaluateRecord no fluxo deste módulo; centraliza contrato reutilizável e preserva validações do chamador. */
...
/** Executa applyAssessment no fluxo deste módulo; centraliza contrato reutilizável e preserva validações do chamador. */
...
/** Executa approveIssue no fluxo deste módulo; centraliza contrato reutilizável e preserva validações do chamador. */
...
/** Executa approvalPlan no fluxo deste módulo; centraliza contrato reutilizável e preserva validações do chamador. */
...
async function github(config, pathname, method, options = {}) {
  ...
  return requestJson({ body: options.body, cachePath: method === "GET" ? path.join(ROOT_DIR, ".ia.rules", "cache", "inbox", `${sha256(pathname).slice(0, 16)}.json`) : "", headers: { "User-Agent": "agents-governance-inbox", ...(options.token ? { Authorization: `Bearer ${options.token}` } : {}) }, maxBytes: config.maxBytes, method, retries: config.retries, timeoutMs: config.timeoutMs, url: `${apiBase}${pathname}` });
}
...
function readConfig() {
  ...
  const localPath = path.join(ROOT_DIR, ".ia.rules", "upstream.json");
  const localConfig = fs.existsSync(localPath) ? readJson(localPath) : {};
  return { ...packageConfig, ...localConfig };
}

/** Executa readIndex no fluxo deste módulo; centraliza contrato reutilizável e preserva validações do chamador. */
...
/** Executa sanitizeIssueText no fluxo deste módulo; centraliza contrato reutilizável e preserva validações do chamador. */
...
/** Executa normalizeLabels no fluxo deste módulo; centraliza contrato reutilizável e preserva validações do chamador. */
...
/** Executa sanitizeLogin no fluxo deste módulo; centraliza contrato reutilizável e preserva validações do chamador. */
function sanitizeLogin(value) { const login = String(value || ""); return /^[A-Za-z0-9-]{1,39}$/u.test(login) ? login : ""; }
/** Executa detectLanguage no fluxo deste módulo; centraliza contrato reutilizável e preserva validações do chamador. */
...
/** Executa localizedReason no fluxo deste módulo; centraliza contrato reutilizável e preserva validações do chamador. */
function localizedReason(reason, language) { return /^pt/iu.test(language) ? `Motivo: ${reason}.` : `Reason: ${reason}.`; }
/** Executa publicIssueUrl no fluxo deste módulo; centraliza contrato reutilizável e preserva validações do chamador. */
...
/** Executa isoDate no fluxo deste módulo; centraliza contrato reutilizável e preserva validações do chamador. */
...
/** Executa recordFile no fluxo deste módulo; centraliza contrato reutilizável e preserva validações do chamador. */
...
/** Executa resolveInboxDir no fluxo deste módulo; centraliza contrato reutilizável e preserva validações do chamador. */
...
/** Executa validateAssessment no fluxo deste módulo; centraliza contrato reutilizável e preserva validações do chamador. */
...
/** Executa positiveInt no fluxo deste módulo; centraliza contrato reutilizável e preserva validações do chamador. */
...
/** Executa validRepository no fluxo deste módulo; centraliza contrato reutilizável e preserva validações do chamador. */
...
/** Executa writeJson no fluxo deste módulo; centraliza contrato reutilizável e preserva validações do chamador. */
...
/** Executa absolute no fluxo deste módulo; centraliza contrato reutilizável e preserva validações do chamador. */
function absolute(file) { return path.isAbsolute(file) ? file : path.resolve(ROOT_DIR, file); }
/** Executa relative no fluxo deste módulo; centraliza contrato reutilizável e preserva validações do chamador. */
...
/** Executa selfTest no fluxo deste módulo; centraliza contrato reutilizável e preserva validações do chamador. */
function selfTest() {
