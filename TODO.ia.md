...
* [ ] Corrigir espaçamentos e parametrização das Declarações Unificadas
  ...
  * Inserir espaçamento vertical adequado entre a linha de **local e data** e a descrição de cabeçalho contendo os documentos vinculados.
  * `APLICA-SE A TODAS AS CONTAS PJ/PF DO(S) DECLARANTE(S).` NÃO DEVE quebrar linha/parágrafo: DEVE permanecer integrado ao mesmo parágrafo imediatamente anterior.
  * `Página 3 de 6` — e equivalentes dinâmicos — também NÃO DEVE quebrar linha/parágrafo.
  * Exceto **local e data**, todo o cabeçalho DEVE formar um único parágrafo contínuo, estilo ata, sem quebras voluntárias de linha/parágrafo e com alinhamento **justificado**.
  * O template integral desse parágrafo, caso ainda não seja parametrizado, DEVE ser personalizável por configuração centralizada.
  * O template DEVE utilizar o mesmo mecanismo já adotado para o texto de assinatura, com marcadores/substituições inteligentes para valores dinâmicos, conceitualmente como `Uma frase com ${valor}`.
...
* [ ] Corrigir dimensionamento semântico das tabelas nas Declarações Unificadas
  ...
  * As larguras DEVEM se ajustar ao conteúdo efetivo das colunas, evitando:

    * colunas simples excessivamente largas;
    * compressão indevida de conteúdo textual;
    * quebras de linha evitáveis;
    * crescimento vertical desnecessário.
  * O objetivo NÃO é espremer conteúdo: preservar pequeno padding interno suficiente para impedir texto encostado nas bordas.
...
* [ ] Adicionar mesclagem opcional ao Conversor de modelos CSV
  ...
  * O CSV de `mesclar` destina-se a acrescentar colunas complementares ao resultado já processado, preservando associação correta e evitando duplicação, redundância, transbordamento ou vinculação indevida.
  * `mesclar` DEVE possuir **exatamente um** indexador comum dentre:

    * uma coluna existente em `Identificadores por prioridade; inclua id apenas para preserva-lo`;
    * ou `telefone`/`fone`, conforme nomes efetivamente suportados.
  * Se `mesclar` contiver simultaneamente mais de um desses indexadores, considerar a associação ambígua/potencialmente colidente, exibir aviso inequívoco e **impedir a execução**.
  * Com um único indexador válido, executar o processamento já existente e aplicar posteriormente associação equivalente a `LEFT JOIN` entre:

    * `resultado prévio`;
    * `mesclar`;
    * usando o indexador comum;
    * sem duplicar a coluna indexadora de `mesclar`.
  * No modo padrão:

    * preservar somente as linhas do `resultado prévio`;
    * acrescentar todas as demais colunas de `mesclar` quando houver correspondência;
    * linhas de `mesclar` sem correspondência NÃO DEVEM criar novas linhas;
    * linhas do `resultado prévio` sem correspondência DEVEM permanecer.
  * Próximo ao campo `mesclar`, disponibilizar seleção visual em estilo `switch`/radio mutuamente exclusiva para escolher a política de linhas resultantes:

    * **Resultado prévio** — padrão; semântica equivalente a `LEFT JOIN` descrita acima;
    * **Somente mesclar** — preservar apenas as linhas originárias de `mesclar`, incorporando os dados correspondentes do `resultado prévio`;
    * **Somadas** — preservar a união das linhas existentes em ambos, incluindo linhas presentes em apenas um dos lados, sem duplicação indevida das correspondências.
  * A seleção DEVE alterar somente a política de preservação de linhas; associação de colunas continua baseada exclusivamente no indexador comum válido.
  * Em qualquer modo:

    * uma chave NÃO PODE receber dados de outra;
    * a coluna indexadora de `mesclar` NÃO DEVE ser duplicada;
    * correspondências DEVEM permanecer determinísticas;
    * conflitos/ambiguidades de associação NÃO DEVEM ser resolvidos silenciosamente.
  * Após a mesclagem, o conjunto resultante passa a constituir o **resultado definitivo** para processamento/exportação subsequente.
  * A mesclagem DEVE permanecer estágio adicional **opcional**, sem substituir, reduzir ou regredir o fluxo atual do Conversor.
  * Inspecionar previamente os contratos, transformações e validações já existentes e aderir integralmente ao RCF e `AGENTS.md` vigentes.
